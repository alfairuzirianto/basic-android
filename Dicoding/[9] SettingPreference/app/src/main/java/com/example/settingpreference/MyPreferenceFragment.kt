...
class MyPreferenceFragment : PreferenceFragmentCompat(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        if (key == NAME) namePreference.summary = sharedPreferences?.getString(NAME, DEFAULT_VALUE)
        if (key == EMAIL) emailPreference.summary = sharedPreferences?.getString(EMAIL, DEFAULT_VALUE)
        if (key == AGE) agePreference.summary = sharedPreferences?.getString(AGE, DEFAULT_VALUE)
        if (key == PHONE) phonePreference.summary = sharedPreferences?.getString(PHONE, DEFAULT_VALUE)
        if (key == LOVE) isLoveMuPreference.isChecked = sharedPreferences!!.getBoolean(LOVE, false)
    }
}